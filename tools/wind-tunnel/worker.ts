...
async function prepareSource(run: RunState) {
  if (run.spec.source.githubSha !== SOURCE_SHA) {
    throw new Error(`Worker source mismatch: run=${run.spec.source.githubSha} worker=${SOURCE_SHA}`);
  }
  ...
  const experimentRaw = await readFile(path.join(APP_DIR, run.spec.experiment.name), 'utf8');
  const taskRaw = await readFile(path.join(APP_DIR, run.spec.task.path), 'utf8');
  if (sha256(experimentRaw) !== run.spec.experiment.digest) throw new Error('Experiment definition digest does not match frozen RunSpec');
  if (sha256(taskRaw) !== run.spec.task.digest) throw new Error('Task digest does not match frozen RunSpec');
  ...
  const init = await runShell([
    ...
    'git add -A',
    `git commit -qm "workspace baseline ${run.spec.source.githubSha}"`,
  ].join(' && '), sourceDir, 120_000);
  if (init.exitCode !== 0) throw new Error(`Workspace baseline failed: ${init.stderr || init.stdout}`);
  const head = await runShell('git rev-parse HEAD', sourceDir, 30_000);
  if (head.exitCode !== 0) throw new Error(head.stderr || head.stdout);
  return {root, sourceDir, baselineCommit: head.stdout.trim(), experiment, task: taskRaw};
}
...
async function executeRun(claimed: RunState) {
  ...
  try {
    ...
    await appendEvent({runId: state.runId, type: 'run.prepared', at: new Date().toISOString(), data: {workspaceBaselineCommit: prepared.baselineCommit, workerId: WORKER_ID}});
    ...
    const runArm = async (arm: Arm) => {
      ...
      try {
        ...
        const finishedAt = new Date().toISOString();
        await putArtifact({runId: state.runId, arm, type: 'arm.error', name: 'error.txt', mediaType: 'text/plain', content: message});
...
async function main() {
  ...
  console.log(`Immutable source SHA: ${SOURCE_SHA}`);
  while (true) {
    try {
