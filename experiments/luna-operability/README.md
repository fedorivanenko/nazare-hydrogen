...
```json
{
  "allow": ["read", "edit", "write", "apply_patch", "nazare_find", "nazare_inspect", "nazare_compile"],
...
`bash` is intentionally unavailable during this task: worker owns full verification, preventing model from spending fixed agent budget on duplicate gates. Worker first executes pinned `.wind-tunnel/prepare-change.ts` and gives model bounded capability context plus focused source excerpts. Pinned `apply_patch` supports coordinated unified-diff edits. Nazare tools then expose registry search, entity-neighborhood inspection, and capability-change compilation for model follow-up. Primary experiment uses `openai/gpt-oss-120b`; `gpt-oss-20b` remains lower-capacity comparison baseline.
